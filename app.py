...
import requests
from duckduckgo_search import DDGS # 👈 Jdida - Web Search
...
# FONCTION WEB SEARCH JDIDA 👑
def search_web(query, max_results=3):
    """Search f internet b DuckDuckGo"""
    try:
        with DDGS() as ddgs:
...
def get_supplier_info(supplier_name):
    """Jib infos 3la fournisseur men internet"""
    results = search_web(f"{supplier_name} fournisseur plastique Maroc", max_results=2)
    if results:
        info = f"**🌐 Infos trouvées sur {supplier_name}:**\n\n"
        for r in results:
            info += f"• **{r['title']}**\n{r['body'][:150]}...\n[Source]({r['href']})\n\n"
        return info
    return f"Ma l9itch infos 3la {supplier_name} f internet"

def get_mp_price(matiere_premiere):
    """Jib prix actuel dyal matière première"""
    results = search_web(f"prix {matiere_premiere} 2026 EUR kg", max_results=2)
    if results:
        info = f"**💰 Prix actuel {matiere_premiere}:**\n\n"
        for r in results:
            info += f"• {r['body'][:200]}...\n[Source]({r['href']})\n\n"
        return info
    return f"Ma l9itch prix actuel dyal {matiere_premiere}"
...
if fichier_conso and fichier_param:
        ...
        with st.spinner("⏳ Analyse MRP + Prophet + Écarts..."):
            ...
            for i, code_mp in enumerate(liste_mp):
                try:
                    ...
                    df_mp = df_mp.dropna(subset=['Date']).sort_values('Date')
            ...
            if resultats_globaux:
                ...
                if len(df_non_aligne) > 0:
                    ...
                    fig_alertes.add_trace(go.Bar(x=df_non_aligne['Code_MP'], y=df_non_aligne['Stock_Actuel_kg'], name='Stock Actuel (kg)', marker_color='#ff9999'))
                    fig_alertes.add_trace(go.Bar(x=df_non_aligne['Code_MP'], y=df_non_aligne['Couverture_j'], name='Couverture (jours)', marker_color='#ff4444', yaxis='y2'))
                    fig_alertes.add_trace(go.Scatter(x=df_non_aligne['Code_MP'], y=df_non_aligne['Lead_Time_j'], name='Lead Time Requis (j)', mode='lines+markers', line=dict(color='#000', width=3, dash='dash'), yaxis='y2'))
                    fig_alertes.update_layout(title="⚠️ Stock vs Couverture vs Lead Time", xaxis=dict(title="Code MP"), yaxis=dict(title="Stock (kg)", side='left'), yaxis2=dict(title="Jours", side='right', overlaying='y'), barmode='group', height=450)
                ...
                with tab1:
                    ...
                    fig = go.Figure()
                    fig.add_trace(go.Bar(name='MRP Production', x=df_comp['Code_MP'], y=df_comp['Besoin_MRP_kg'], marker_color='#0066cc', text=df_comp['Besoin_MRP_kg'].round(0).astype(int).astype(str), textposition='auto'))
                    fig.add_trace(go.Bar(name='Prévision Prophet', x=df_comp['Code_MP'], y=df_comp['Prevision_Prophet_kg'], marker_color='#66b3ff', text=df_comp['Prevision_Prophet_kg'].round(0).astype(int).astype(str), textposition='auto'))
                    fig.update_layout(barmode='group', title="Comparaison Besoins MRP vs Prévision - Tous les MPs", xaxis_title="Code MP", yaxis_title="Quantité (kg)", height=450)
                ...
                with tab3:
                    ...
                    with col1:
                        fig = px.pie(df_f_count, values='Nb_MP', names='Fournisseur', title="Répartition MPs par Fournisseur", hole=0.3)
                        st.plotly_chart(fig, use_container_width=True)
                    with col2:
                        st.markdown("**Détail par MP:**")
                        st.dataframe(df_plan[['Code_MP', 'Designation', 'Fournisseur', 'Prix_EUR', 'Lead_Time_j']], use_container_width=True, hide_index=True)
    ...
    if save_plan and 'df_resultat' in st.session_state:
        nouveau_plan = {'date': st.session_state['date_generation'], 'nb_mp': len(st.session_state['df_resultat']), 'cout_total': st.session_state['kpis']['cout_total_commande'], 'kpis': st.session_state['kpis'], 'data': st.session_state['df_resultat']}
        st.session_state['historique_plans'].insert(0, nouveau_plan)
        st.success(f"✅ Plan sauvegardé!")
        st.rerun()

# CHAT IA M3A WEB SEARCH 👑
if 'df_resultat' in st.session_state:
    st.divider()

    col_chat_title, col_clear_btn = st.columns([4, 1])
    with col_chat_title:
        st.header("🧠 Assistant MRP ↔ Appro + Web Search 🌐")
    with col_clear_btn:
    ...
    if prompt := st.chat_input("Swel... Ex: Prix actuel MP_PP wla Infos 3la Atlas Plastics"):
        st.session_state['messages'].append({"role": "user", "content": prompt})
        with st.chat_message("user"):
            st.markdown(prompt)

        with st.chat_message("assistant"):
            with st.spinner("🔍 Kan9lb f internet..."):
                df = st.session_state['df_resultat']
                kpis = st.session_state['kpis']
                prompt_lower = prompt.lower()

                # WEB SEARCH QUERIES
                if "prix" in prompt_lower or "cout" in prompt_lower or "price" in prompt_lower:
                    # Extract MP name
                    mp_mentioned = None
                    for mp in df['Code_MP'].values:
                        if mp.lower() in prompt_lower:
                            mp_mentioned = mp
                            break

                    if mp_mentioned:
                        response = get_mp_price(mp_mentioned)
                    else:
                        response = "🔍 **3tini smiya dyal matière première** bach n9lb 3la prix actuel\nExemple: *Prix actuel MP_PP*"

                elif "fournisseur" in prompt_lower or "supplier" in prompt_lower or "atlas" in prompt_lower or "medina" in prompt_lower or "safi" in prompt_lower:
                    # Extract supplier name
                    supplier_mentioned = None
                    for supp in df['Fournisseur'].unique():
                        if supp.lower() in prompt_lower:
                            supplier_mentioned = supp
                            break

                    if supplier_mentioned:
                        response = get_supplier_info(supplier_mentioned)
                    else:
                        # Search general
                        results = search_web(prompt, max_results=3)
                        response = f"🌐 **Résultats Web:**\n\n"
                        for r in results:
                            response += f"• **{r['title']}**\n{r['body'][:200]}...\n[Source]({r['href']})\n\n"

                elif "aligné" in prompt_lower or "jour" in prompt_lower or "mrp" in prompt_lower:
                    ...
                    # Search web b question dyalo
                    results = search_web(prompt, max_results=3)
                    if results:
                        response = f"🌐 **Jbt lik infos men internet:**\n\n"
                        for r in results:
                            response += f"**{r['title']}**\n{r['body'][:250]}...\n[Source]({r['href']})\n\n"
                    else:
...
**Swel 3la:** aligné, écart, critique, prix [MP], fournisseur [nom]"""
