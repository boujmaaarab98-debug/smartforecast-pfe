...
st.set_page_config(page_title="MRP Pro V4.11.1 - Fix", page_icon="🚀", layout="wide")

st.title("🚀 MRP Pro Dashboard V4.11.1 - KPIs Consommation & Prévision")
...
# ==================== 2. FONCTION NETTOYAGE NUMÉRIQUE ====================
def clean_numeric(series):
    """N7wl ay haja l number: n7iydo espaces, virgules, text..."""
    return pd.to_numeric(
        series.astype(str)
       .str.replace(' ', '') # N7iydo espaces: 1 100 → 1100
       .str.replace(',', '.') # Nbdlo virgule b point: 1,5 → 1.5
       .str.replace('€', '') # N7iydo €
       .str.replace('KG', '') # N7iydo KG
       .str.replace('kg', '')
       .str.strip(),
        errors='coerce'
    )
...
def load_full_data():
    try:
        ...
        # Clean numeric
        for col in ['Délai_Param', 'MOQ_Param', 'Stock_Sécu']:
        ...
        # FIX: Clean numeric 9bl ma ndiro *
...
with tab2:
    ...
    fig_trend = px.line(conso_mensuelle_sum, x='Mois', y='Besoin_MP_KG', title="Tendance Consommation Globale - 12 Mois")
